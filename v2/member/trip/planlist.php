...
function build_alarm_mail(array $r): array{
  $body = [];
  $body[] = '以下の山行について、最終下山時刻を過ぎたことをお知らせします。';
  $body[] = '';
...
try {
  ...
  // 最終下山連絡時刻の超過通知（重複防止）
  try{
    $alarmStmt = $pdo->query("SELECT * FROM trips WHERE status='PLAN_SUBMITTED' AND COALESCE(final_contact_deadline,'')<>'' AND (alarm_notified_at IS NULL OR trim(alarm_notified_at)='') ORDER BY final_contact_deadline,id");
    $alarmRows = $alarmStmt ? $alarmStmt->fetchAll(PDO::FETCH_ASSOC) : [];
    foreach($alarmRows as $ar){
      if(!is_over_deadline((string)($ar['status'] ?? ''), (string)($ar['final_contact_deadline'] ?? ''))) continue;

      $ok = false;
      if(trip_notify_method()==='line'){
        if (is_file(__DIR__ . '/line_notify.php')) {
          require_once __DIR__ . '/line_notify.php';
          $days = (string)($ar['date_ymd'] ?? '');
          $dateTo = (string)($ar['date_to'] ?? '');
          if($dateTo!=='' && $dateTo!==$days) $days .= '～'.$dateTo;
          $typeLine = (string)($ar['trip_type'] ?? '');
          $tripStyle = (string)($ar['trip_style'] ?? '');
          if($tripStyle!=='') $typeLine .= '　形態：'.$tripStyle;
          $lines = [];
          $lines[] = '以下の山行について、最終下山時刻を過ぎたことをお知らせします。';
          $lines[] = '日程：'.$days;
          $lines[] = '山名：'.(string)($ar['title'] ?? '');
          if($typeLine!=='') $lines[] = '種別：'.$typeLine;
          $lines[] = 'リーダー：'.(string)($ar['leader'] ?? '');
          $people = trip_format_people(isset($ar['people']) ? (int)$ar['people'] : 0);
          if($people!=='') $lines[] = '人数：'.$people;
          $lines[] = '最終下山連絡時刻：'.(string)($ar['final_contact_deadline'] ?? '');
          if(function_exists('line_push_flex') && defined('LINE_SUBMIT_GROUP_ID') && LINE_SUBMIT_GROUP_ID !== ''){
            $ok = line_push_flex(LINE_SUBMIT_GROUP_ID, '最終下山時刻超過', '【最終下山時刻超過】', $lines, trip_planlist_url((string)($ar['date_ymd'] ?? '')));
          }
        }
      }else{
        $mail = build_alarm_mail($ar);
        $ok = trip_send_mail((string)$mail['to'], (string)$mail['subject'], (string)$mail['body']);
      }

      if($ok){
        $nowAlarm = date('Y-m-d H:i:s');
        $pdo->prepare("UPDATE trips SET alarm_notified_at=:t, updated_at=:u WHERE id=:id")
            ->execute([':t'=>$nowAlarm, ':u'=>$nowAlarm, ':id'=>(int)($ar['id'] ?? 0)]);
        trip_event_log((string)($ar['title'] ?? ''), 'alarm_notify', '最終下山時刻超過通知', (string)($ar['status'] ?? ''), (string)($ar['status'] ?? ''), 'id='.(int)($ar['id'] ?? 0));
      }
    }
  }catch(Throwable $e){
    // 通知失敗で一覧表示自体は止めない
